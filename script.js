...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function filterSlides(filterValue) {
    ...
    setTimeout(() => {
      ...
      const visibleSlides = Array.from(swiperSlides).filter(
        (slide) =>
          slide.style.display !== "none" && !slide.classList.contains("hidden")
      );
      if (visibleSlides.length > 0) {
        const firstVisibleIndex = Array.from(swiperSlides).indexOf(
          visibleSlides[0]
        );
    ...
    if (portfolioItem) {
      ...
      if (img) {
        ...
        img.addEventListener("click", () => {
          ...
          const visibleSlides = Array.from(swiperSlides).filter(
            (s) => s.style.display !== "none" && !s.classList.contains("hidden")
          );
